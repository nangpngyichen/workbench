...
/* ---------- 基础工具 ---------- */
const PREFIX='wb_';
const APP_VER='v87';  // 与 sw.js 的 CACHE 版本保持同步，仅用于首页展示当前代码版本
...
function renderWorkloadList(){
  ...
  box.innerHTML=arr.map(r=>{
    ...
    <div style="font-size:11px;opacity:.7">${isTriple?'🌟三薪日 ｜ ':''}工单${r.ticket} 邮件${r.mail} 备档${r.archive} 不良${r.bad} 物联网${r.iot} ｜ 工作量 ${pF(r.work)}</div>
  ...
  box.innerHTML=years.map(y=>{
    ...
    const body=yearMonths.map(m=>{
      ...
      const detail=recs.length
        ? recs.map(r=>{ const isT=(msched[r.date]&&msched[r.date].shift==='法定三薪日'); return recLine(r.date,'工单 '+pF(num(r.ticket))+' ｜ 积分 '+pF(num(r.points))+' 分'+(isT?' ｜ 🌟三薪日':'')); }).join('')
...
// 每月工资模块「今日挣多少」：仅展示当天（真实日期）这一个金额
// ＝ 分摊固定 ＋ 当天三薪(若今天是法定三薪日) ＋ 当天提成(边际·按累计积分达档，累计到今天)
// 当天提成＝computeCommission(累计到今天含今天) − computeCommission(累计到昨天)，
// 即今天工作量把当月累计积分推到哪一档、超出部分按该档费率计（档一 9000-11000→15/60、档二 11000-15000→18/60、档三 >15000→21/60，均×系数）
function renderTodayEarnings(month, dailyFixed, triple, coef){
  const now=new Date();
  const tY=now.getFullYear(), tM=String(now.getMonth()+1).padStart(2,'0'), tD=String(now.getDate()).padStart(2,'0');
  const todayStr=tY+'-'+tM+'-'+tD;
  if(month!==(tY+'-'+tM)){
    return '<div class="today-earn note">「今日挣多少」仅在选择本月（'+tY+'-'+tM+'）时显示。请切到当前月份查看今天的金额。</div>';
  }
  const sched=load('schedule',{})[month]||{};
  const tripleDays=getScheduleStats(month).tripleDays;
  const dayTriple=tripleDays>0? triple/tripleDays : 0;
  const arr=load('workload',[]).filter(r=>r.date&&r.date.startsWith(month)&&r.date<=todayStr).slice().sort((a,b)=>a.date<b.date?-1:1);
  let cum=0; arr.forEach(r=>{ cum+=num(r.points); });
  const beforeArr=arr.filter(r=>r.date<todayStr); let before=0; beforeArr.forEach(r=>{ before+=num(r.points); });
  const dayCommission=computeCommission(cum,coef).total - computeCommission(before,coef).total;
  const isTriple=sched[todayStr]&&sched[todayStr].shift==='法定三薪日';
  const dTri=isTriple?dayTriple:0;
  const dayWage=dailyFixed+dTri+dayCommission;
  return '<div class="today-earn">'
    +'<div class="today-earn-h">💰 今日挣多少钱（'+todayStr+'）</div>'
    +'<div class="today-earn-body">'
    +recLine('分摊固定工资','¥'+money(dailyFixed)+'（固定组成 ÷ 排班 '+getDivisorDays(month)+' 天）')
    +(isTriple?recLine('当天三薪（法定三薪日）','¥'+money(dTri)):'')
    +recLine('当天提成（累计积分达档·含今天）','¥'+money(dayCommission)+'（累计 '+pF(cum)+' 分，昨日止 '+pF(before)+' 分）')
    +recLine('💰 今日合计','¥'+money(dayWage))
    +'</div></div>';
}
